...
class pozostale{
	...
	echo '<table border="1">';
	echo '<tr>';
	echo '<td width="60px"> <b> Edytuj </td>';
...
while ($i < $num)
{
	...
	$element->naz_pan=mysql_result($wynik,$i,"naz_pan"); 
	$element->email=mysql_result($wynik,$i,"email"); 
	$element->kod_p=mysql_result($wynik,$i,"kod_p"); 
	...
	echo '<td width="150px">'.$naz_pan.'</td>';
...
$prevLink = $_SERVER['index.php?strona=5'] . '?page=' . $prev;
$nextLink = $_SERVER['index.php?strona=5'] . '?page=' . $next;
